...
Pet 클래스를 할당받은 pet1 은 출력용 메서드가 없다. 그래서 출력을 시도하면 출력용 메서드를 일단 한 번 탐색한다.

그렇게 탐색하던 중 Cat 클래스에서 출력용 메서드인 __str__ 을 찾았고 가져와 다시 출력을 시도한다.

그런데 이번에는 str 메서드 안에 있는 문자열에서 f-string 이 걸려있는 self.sound 에 대한 값이 클래스 안에 없는 걸 발견하고 다시 이를 찾아 두번째 탐색을 시작한다.

그러던 중 Dog 클래스에서 클래스 변수인 sound 를 찾아냈고 이를 가져와 출력에 사용한다.

개인적으로 메서드를 cat 에서 가져왔는데 왜 출력은 멍멍이 나오는거지? 라고 머리를 싸매긴 했는데

이유를 알고 나니 파이썬의 작동원리에 대한 개념이 한결 나아진 것 같다.